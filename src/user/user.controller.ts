...
export class UserController {
    ...
    async login(@Body() body: LoginDto, @Res() res: Response) {
        try {
            ...
            res.status(200).send({ message: 'Usuario autenticado', accessToken: response.accessToken, currentUser: response.user })
        } catch (error) {
    ...
    async registerAccount(@Body() body: RegisterAccountDto, @Req() req: any, @Res() res: Response) {
        try {
            const response = await this.userService.registerAccount(body, req)
            res.status(HttpStatus.OK).send({ message: 'Usuario registrado con éxito' })
        } catch (error) {
            res.status(HttpStatus.BAD_REQUEST).send({ error, message: error?.response?.data?.response })
        }
    }
