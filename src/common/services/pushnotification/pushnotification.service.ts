...
export class PushnotificationService {
  private url1 = 'api/pushnotifications/send-push';
  private url2 = 'api/pushnotifications/send-pushnotification';
  ...
  sendPushnotification(request: IRequestPushnotification) {
    ...
    const apiUrl = `${MS_PUSHNOTIFICATION}${this.url1}`;
  ...
  sendPushnotificationWithoutTemplate(request: IRequestPushnotification) {
    const { MS_PUSHNOTIFICATION, PK_MS_PUSHNOTIFICATION } = process.env;
    const apiUrl = `${MS_PUSHNOTIFICATION}${this.url2}`;
    return firstValueFrom(
      this.httpService.post(apiUrl, request, {
        headers: {
          Authorization: `Bearer ${PK_MS_PUSHNOTIFICATION}`,
